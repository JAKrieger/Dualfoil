...
c     minor modifications, January 2021
...
c      January, 2021:
c        The following changes have been made to ensure numerical stability
c        - Avoid division by rr=0, (dividing by 1 if rr is 0), (line 2049+)
c        - Avoid negative array indices for xt , (line 2728)
...
c and for a rigorous energy balance.
     ...
     &/(rr+izfl)*omi-sumpa(mpa,ii))*Rad1pa(mpa)
      if (mvdc1.eq.1) pwfindiv(mpa,ii)=xx(kj,ii)
      pwfindiv_d(mpa,ii)=-Rad1pa(mpa)*ai2(1,mpa)/(rr+izfl)*omi
      elseif (ii.ge.n1+n2) then
      pwfindiv(mpa,ii)=(ai(1,mpa)*(xt(2+mpa,ii,kk-1+kadd)-c(2+mpa,ii))
     &/(rr+izfl)*omi-sumpa(mpa,ii))*Rad3pa(mpa)
      if (mvdc3.eq.1) pwfindiv(mpa,ii)=xx(kj,ii)
      pwfindiv_d(mpa,ii)=-Rad3pa(mpa)*ai(1,mpa)/(rr+izfl)*omi
      else
